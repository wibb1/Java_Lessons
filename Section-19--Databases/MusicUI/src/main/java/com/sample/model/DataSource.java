...
import static com.sample.model.QueryStringBuilder.*;
...
public class DataSource {
    ...
    private PreparedStatement queryAlbumsByArtistsId;
    ...
    public boolean open() {
        try {
            ...
            queryAlbumsByArtistsId = conn.prepareStatement(QUERY_ALBUMS_BY_ARTIST_ID);
            return true;
    ...
    public void close() {
        try {
            ...
            if (queryAlbumsByArtistsId != null) {
                queryAlbumsByArtistsId.close();
            }
    ...
    public List<Album> queryAlbumsByArtistId(int id) {
        try {
            queryAlbumsByArtistsId.setInt(1, id);
            ResultSet resultSet = queryAlbumsByArtistsId.executeQuery();

            List<Album> albums = new ArrayList<>();
            while (resultSet.next()) {
                Album album = new Album();
                album.setId(id);
                album.setArtistId(resultSet.getInt(1));
                album.setName(resultSet.getString(2));
                albums.add(album);
            }
            return albums;
        } catch (Exception e) {
            printSQLErrorMessage(e, "Error in queryAlbumsByArtistId");
            return null;
        }
    }
    ...
    public List<Album> findAlbumsByArtist(String artistName) {
        ...
        try (Statement statement = conn.createStatement();
            ...
            while (resultSet.next()) {
                Album album = new Album();
                album.setArtistId(resultSet.getInt(ARTIST_ALBUM_INDEX));
